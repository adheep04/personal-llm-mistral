...
class Transformer(ModelBase, LoRALoaderMixin):
    def __init__(
        ...
        assert nbits in [8, 4, 3, 2, 1], "invalid nbits"
        assert gsize in [16, 32, 64, 128, 256], "invalid group size"
        quant_config = BaseQuantizeConfig(nbits=nbits, group_size=gsize)
        ...
        num_layers_per_rank = math.ceil(self.n_layers / 1)
        offset = 0 * num_layers_per_rank
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: bool = True, assign: bool = False) -> None:
        ...
        for k, v in state_dict.items():
            if (
            k.startswith("tok_embeddings") or \
            k.startswith("norm") or \
            k.startswith("output") or \
            k.startswith("vision_encoder") or \
            k.startswith("vision_language_adapter")
            ):
                state_to_load[k] = v
    ...
    def from_folder(
        ...
        assert safetensors_model_file.exists(), "safetensor file missing"
